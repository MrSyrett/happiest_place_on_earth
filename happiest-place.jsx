...
const DL_RAW = [
  ...
  ["roger", "Roger Rabbit's Car Toon Spin", "Toontown", "ride", "D", 6, 30, 11, -3, 0, 0, 679, 107],
  ["gadget", "Gadget's Go Coaster", "Toontown", "ride", "B", 3, 20, 6, -2, 0, 0, 546, 106],
  ...
  ["dumbo", "Dumbo the Flying Elephant", "Fantasyland", "ride", "C", 2, 35, 10, -2, 0, 0, 598, 273],
  ["carrousel", "King Arthur Carrousel", "Fantasyland", "ride", "B", 3, 15, 7, 0, 0, 0, 593, 311],
  ["pinocchio", "Pinocchio's Daring Journey", "Fantasyland", "ride", "C", 4, 20, 8, -1, 0, 0, 563, 308],
  ["caseyjr", "Casey Jr. Circus Train", "Fantasyland", "ride", "B", 4, 15, 7, 2, 0, 0, 561, 275],
  ...
  ["duckpond", "Donald's Duck Pond", "Toontown", "ride", "A", 10, 5, 5, -3, 0, 0, 583, 112],
  ["chiptree", "Chip 'n' Dale's Treehouse", "Toontown", "ride", "A", 6, 5, 5, -3, 0, 0, 549, 84],
  ...
  ["buzz", "Buzz Lightyear Astro Blasters", "Tomorrowland", "ride", "C", 8, 25, 10, -2, 0, 0, 702, 439],
  ["nemo", "Finding Nemo Submarines", "Tomorrowland", "ride", "C", 15, 35, 10, 4, 0, 0, 791, 373],
  ["autopia", "Autopia", "Tomorrowland", "ride", "C", 10, 40, 8, -2, 0, 0, 821, 414],
  ["orbitor", "Astro Orbitor", "Tomorrowland", "ride", "B", 5, 25, 7, -3, 0, 0, 652, 441],
...
function comfortDelta(a, weather, heat) {
  // a soaking is a relief in real heat and miserable otherwise
  if (WET.has(a.id)) return weather === "drizzle" ? -10 : Math.round(-2 + 20 * (heat || 0));
...
function comfortDrain(weather, t) {
  return (weather === "drizzle" ? 0.13 : 0.05) + 0.16 * heatAt(weather, t);
}
...
function tempAt(weather, t) {
  const w = WEATHERS[weather] || WEATHERS.clear;
  const h = ((480 + t) / 60) % 24;
  const mid = (w.hi + w.lo) / 2, amp = (w.hi - w.lo) / 2;
  return Math.round(mid + amp * Math.cos((2 * Math.PI * (h - 15)) / 24));
}
// 0 when it's pleasant, 1 when it's punishing. Drives energy and comfort loss.
function heatAt(weather, t) {
  return Math.max(0, Math.min(1, (tempAt(weather, t) - 76) / 22));
}
const weatherLabel = (weather, t) => `${WEATHERS[weather].label} · ${tempAt(weather, t)}°`;
...
export default function HappiestPlace() {
  ...
  function passTime(mins) {
    const hot = 1 + 0.55 * heatAt(seed.weather, t);
    setEnergy((e) => clamp(e - (mins / 10) * 0.3 * hot * drain));
    setFuel((f) => clamp(f - (mins / 10) * 0.95));
    setComfort((c) => clamp(c - mins * comfortDrain(seed.weather, t) * drain));
  ...
  function beginAttraction(raw, opt) {
    ...
    gain -= wallet - a.cost < DIFFICULTY[mode].budget * 0.15 ? 3 : 0;   // scales with your budget
    ...
    setRun({
      ...
      comfort: comfortDelta(a, seed.weather, heatAt(seed.weather, t)),
  ...
  if (screen === "end")
    return <End joy={joy} t={t} wallet={wallet} energy={energy} comfort={comfort} visited={visited} seed={seed} mode={mode} onAgain={start} onTitle={() => setScreen("title")} />;
      ...
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 9 }}>
          ...
          <span style={{ fontSize: 13.5, color: C.greyLt, fontWeight: 600 }}>
            {weatherLabel(seed.weather, t)} · {seed.crowd > 1.25 ? "Packed" : seed.crowd > 1.0 ? "Busy" : seed.crowd > 0.78 ? "Moderate" : "Light"}
          </span>
        </div>
...
/* ---------------- title & end ---------------- */
function Title({ onStart, onSetup, ready, mode, setMode }) {
  const budget = DIFFICULTY[mode].budget;
  return (
    <Shell>
      <div style={{ flex: 1, overflowY: "auto", padding: "52px 24px 30px", textAlign: "center" }}>
        ...
        <p style={{ fontSize: 16.5, lineHeight: 1.55, color: C.text, maxWidth: 330, margin: "0 auto 22px" }}>
          Rope drop is at 8:00 AM and you have ${budget} in your pocket. Tap a pin to walk there and get in line.
...
function End({ joy, t, wallet, energy, comfort, visited, seed, mode, onAgain, onTitle }) {
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "44px 24px 30px", textAlign: "center" }}>
        ...
        <div style={{ background: C.gap, borderRadius: 14, padding: "6px 16px", textAlign: "left", maxWidth: 340, margin: "0 auto 26px" }}>
          {[["Difficulty", DIFFICULTY[mode].label], ["Attractions ridden", rides],
            ...
            ["Conditions", `${weatherLabel(seed.weather, Math.min(t, CLOSE))}, ${seed.crowd > 1.25 ? "packed" : seed.crowd > 1.0 ? "busy" : seed.crowd > 0.78 ? "moderate" : "light"}`]]
            .map(([k, val]) => (
